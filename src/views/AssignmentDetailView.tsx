...
const AssignmentDetailView = () => {
  ...
  const course = useAppSelector((state) => {
    return state.course.courses.find((course) => {
      return (
        course.assignments.findIndex((assign) => assign._id === _id) !== -1
      );
    });
  });
  const user = useAppSelector((state) => state.user);
  const judgement = useAppSelector(
    (state) =>
      state.judgement.judgements[course?._id as string]?.[_id as string]
  );
    ...
    return [
      {
        ...
        onRender(
          ...
          return (
            <Text style={{ fontWeight: FontWeights.bold }}>{item.name}</Text>
          );
        },
      },
      {
    ...
    return user?.identity === 0
      ? [
          ...
          },
          {
            ...
            name: "提交状态",
            value:
              detail?.state === 1 || detail?.state === 2 ? "已提交" : "未提交",
          },
          {
            key: "score_state",
            name: "评分状态",
          ...
          },
          {
            ...
            name: "剩余时间",
            value:
              new Date(detail?.endTime ?? "") <= new Date()
                ? "已过期"
                : moment(detail?.endTime).fromNow(true),
          },
        ]
      : [];
  }, [detail?.endTime, detail?.scored, detail?.state, user?.identity]);

  const teacherItems: Item[] = useMemo(() => {
      ...
      ? [
          {
            ...
            name: "总人数",
            value: (
              judgement?.submitted.length + judgement?.unsubmitted
            ).toString(),
          },
          {
            key: "submitted",
          ...
          },
          {
            ...
            name: "剩余时间",
            value:
              (+new Date() > +new Date(detail?.endTime as string)
                ? "已过期"
                : "") + moment(detail?.endTime).fromNow(true),
          },
        ]
    ...
    detail?.endTime,
    judgement?.submitted.length,
    judgement?.unsubmitted,
    user?.identity,
  ]);
  ...
  useEffect(() => {
    if (detail?.submissions && detail.state !== 2 && user?.identity === 0) {
      const files = detail?.submissions.map((submission) => {
        return fetch(submission.url)
          .then((res) =>
            Promise.all([res.blob(), res.headers.get("content-type")])
          )
          .then(([blob, type]) => {
  ...
  }, [detail?.submissions, detail?.state, user?.identity]);
    ...
    <section style={{ padding: "40px" }}>
      <Stack style={animationStyle}>
        <Stack.Item>
          ...
          <Text
            variant="xLarge"
            style={{ ...subtitleStyle, display: "block", margin: 0 }}
          >
            开始日期: {detail?.startTime}
          </Text>
          <Text
            variant="xLarge"
            style={{ ...subtitleStyle, display: "block", margin: 0 }}
            ...
            {detail?.state !== 3 && detail?.state !== 2 && (
              <Stack.Item>
                <TitledCard
                  ...
                  actions={
                    uploaderDisabled ? (
                      <PrimaryButton onClick={() => setUploaderDisabled(false)}>
                        修改
                      </PrimaryButton>
                    ) : (
        ...
        {user.identity === 1 && (
          <Stack.Item>
            <TitledCard
              ...
              actions={
                <PrimaryButton
                  onClick={() => navigate(`/judgement/detail/${_id}`)}
                >
                  评判任务
                </PrimaryButton>
              }
